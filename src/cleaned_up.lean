...
def fg_limit_to_module : module.direct_limit (λ (N : fin_submodule R M), N) (le_system) →ₗ[R] M
...
#check finset_to_fin_submodule R ({x} : finset M)

#check elt_to_fin_submodule 

@[simp]lemma finset_to_fin_submodule_eq (x : M) : (finset_to_fin_submodule R ({ x } : finset M)).val = submodule.span R ({ x } : finset M) 
:= by rw finset_to_fin_submodule

#check finset.coe_singleton x 

@[simp]lemma stupidlemma (x : M) : x ∈ (elt_to_fin_submodule R x).val :=
begin
rw[elt_to_fin_submodule, finset_to_fin_submodule],
change x ∈ span R ↑{x},
rw finset.coe_singleton,
exact_mod_cast submodule.mem_span_singleton_self x
end
